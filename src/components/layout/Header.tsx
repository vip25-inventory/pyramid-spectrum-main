        ...
        <div className="header-inner">
          ...
          <nav className={`main-navigation ${showMobileMenu ? 'show' : ''}`}>
            <ul className="nav-menu">
              ...
              <li className={`menu-item has-submenu ${showCourses ? 'active' : ''}`}
                  onMouseEnter={() => handleMenuMouseEvents(true, setShowCourses)}
                  onMouseLeave={() => handleMenuMouseEvents(false, setShowCourses)}
                  onClick={() => handleSubmenuToggle(showCourses, setShowCourses, 'courses')}>
                <Link to="/courses">Courses</Link>
                {(showCourses || isMobile) && (
                  <div className={`submenu ${showCourses ? 'active' : ''}`}>
                    <ul className="submenu-list">
                      <li className={`submenu-item ${activeSubmenuItems['spectrum-courses'] ? 'active' : ''}`}
                          onClick={(e) => { e.stopPropagation(); handleSubItemToggle('spectrum-courses'); }}>
                        <Link to="/courses/spectrum" onClick={(e) => isMobile ? e.preventDefault() : null}>
                          <span className="fixed-width-container">Spectrum</span> <i className="fas fa-chevron-right"></i>
                        </Link>
                        <div className={`submenu-dropdown ${activeSubmenuItems['spectrum-courses'] ? 'active' : ''}`}
                             onClick={(e) => e.stopPropagation()}>
                          <ul>
                            <li><Link to="/courses/spectrum/neet" onClick={handleMobileLinkClick}><span className="fixed-width-container">NEET</span></Link></li>
                            <li><Link to="/courses/spectrum/iit-jee" onClick={handleMobileLinkClick}><span className="fixed-width-container">IIT-JEE</span></Link></li>
                          </ul>
                        </div>
                      </li>
                      <li className={`submenu-item ${activeSubmenuItems['pyramid-courses'] ? 'active' : ''}`}
                          onClick={(e) => { e.stopPropagation(); handleSubItemToggle('pyramid-courses'); }}>
                        <Link to="/courses/pyramid" onClick={(e) => isMobile ? e.preventDefault() : null}>
                          <span className="fixed-width-container">Pyramid</span> <i className="fas fa-chevron-right"></i>
                        </Link>
                        <div className={`submenu-dropdown ${activeSubmenuItems['pyramid-courses'] ? 'active' : ''}`}
                             onClick={(e) => e.stopPropagation()}>
                          <ul>
                            <li className={`submenu-item ${activeSubmenuItems['spoken-english-courses'] ? 'active' : ''}`}
                                onClick={(e) => { e.stopPropagation(); handleSubItemToggle('spoken-english-courses'); }}>
                              <Link to="/courses/pyramid/english" onClick={(e) => isMobile ? e.preventDefault() : null}>
                                <span className="fixed-width-container">Spoken English</span> <i className="fas fa-chevron-right"></i>
                              </Link>
                              <div className={`submenu-dropdown ${activeSubmenuItems['spoken-english-courses'] ? 'active' : ''}`}
                                  onClick={(e) => e.stopPropagation()}>
                                <ul>
                                  <li><Link to="/courses/pyramid/english/basic" onClick={handleMobileLinkClick}><span className="fixed-width-container">Basic English</span></Link></li>
                                  <li><Link to="/courses/pyramid/english/advanced" onClick={handleMobileLinkClick}><span className="fixed-width-container">Advanced English</span></Link></li>
                                  <li><Link to="/courses/pyramid/english/practical" onClick={handleMobileLinkClick}><span className="fixed-width-container">Practical English</span></Link></li>
                                </ul>
                              </div>
                            </li>
                            <li className="menu-direct-link"><Link to="/courses/pyramid/foundation" onClick={handleMobileLinkClick}><span className="fixed-width-container">Foundation | Academics</span></Link></li>
                            <li className="menu-direct-link"><Link to="/courses/pyramid/power-program" onClick={handleMobileLinkClick}><span className="fixed-width-container">Power Program</span></Link></li>
                          </ul>
                        </div>
                      </li>
                    </ul>
              ...
              <li className="menu-item">
                <Link to="/test-series" onClick={handleMobileLinkClick}>Test Series</Link>
              </li>

              <li className="menu-item">
                <Link to="/results" onClick={handleMobileLinkClick}>Results</Link>
              </li>

              <li className="menu-item">
                <Link to="/study-materials" onClick={handleMobileLinkClick}>Study Materials</Link>
              </li>
            ...
            {isMobile && (
              <div className="header-actions mobile-header-actions">
                <Link to="/course-registration" className="btn-callback" title="Register" aria-label="Register" onClick={handleMobileLinkClick}>
                  <img src="/form-6.png" alt="Register" style={{ width: "16px", height: "16px" }} />
                  <span>Register</span>
                </Link>
                <Link to="/contact" className="btn-callback" title="Request a call back" aria-label="Request a call back" onClick={handleMobileLinkClick}>
